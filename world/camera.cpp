...
void Camera::calculate_visable_tiles() {
    ...
    visable_max = center + num_tiles;
    visable_min = center - num_tiles;
}

Vec<float> Camera::world_to_screen(const Vec<float> &world_posistion) const {
    // world coordenates (pos y is up) -> screen coords (pos y is down)
    Vec<float> pixel = (world_posistion - location) * static_cast<float>(tilesize);
...
void Camera::render(const Tilemap &tilemap) const {
    int xmin = std::max(0, visable_min.x);
    int ymin = std::max(0, visable_min.y);
    int xmax = std::min(visable_max.x, tilemap.width - 1);
    int ymax = std::min(visable_max.y, tilemap.height - 1);
